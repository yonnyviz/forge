...
const WORKFLOW_AGENTS_FILE = "AGENTS.md";
const AGENTS_MARKER_START = "<!-- forge:agents-guide start -->";
const AGENTS_MARKER_END = "<!-- forge:agents-guide end -->";
...
function createWorkflowRecord(initPath, input = {}) {
  ...
  writeAgentsGuide(initPath, metadata, briefContext, memoryContext);

  return { metadata, brief: briefContext, memory: memoryContext };
}

function renderAgentsGuide(metadata, brief, memory) {
  ...
  return [
    ...
    "",
    "Forge initiative workspace. This file is generated by Forge; edit content outside the",
    "markers only — anything between them is overwritten on the next Forge update.",
    "",
    "## 🎯 Outcome",
    "",
    brief.outcome || "To be defined",
    "",
    "## 📂 Workspace files",
    "",
    "| Path | Purpose | Agent rule |",
    "|---|---|---|",
    `| \`${metadata.documents.brief}\` | Stable intent: outcome, scope, constraints, definition of done | Read first. Change only when intent changes |`,
    `| \`${metadata.documents.memory}\` | Live state: status, next action, blockers, decisions | Read second. Update at the end of every work session |`,
    `| \`${metadata.documents.outputs}\` | Deliverables produced by this initiative | Write artifacts here |`,
    "| `sessions/<date>_<name>/notes.md` | Per-session notes | Append during the session |",
    "| `.forge/metadata.json` | Forge record (status, phase, agent index) | Do not edit by hand; use Forge commands |",
    "| `.forge/sessions.log` | Session history | Append-only, managed by Forge |",
    "",
    "Both `brief.md` and `memory.md` start with a `<!-- forge-context ... -->` JSON block.",
    "Keep that block valid JSON; prose goes after it.",
    "",
    "## 🔁 Working protocol",
    "",
    `1. Read ${metadata.agent.readOrder.join(" then ")} before acting.`,
    "2. Confirm the next action below still matches the request; ask before changing scope.",
    "3. Do the work; put deliverables in `outputs/`.",
    "4. Update `memory.md` (status, next action, blockers, decisions) before ending.",
    "",
    "## 📍 Current state",
    "",
    `- Status: ${metadata.status} | Phase: ${metadata.phase}`,
    `- Next action: ${memory.nextAction || "Define the next action."}`,
    `- Blockers: ${asStringList(memory.blockers).join("; ") || "none"}`,
    "",
    "## ✅ Definition of done",
    "",
    list(brief.definitionOfDone, "Not defined yet — clarify with the user."),
    "",
    "## 🚧 Constraints",
    "",
    list(brief.constraints, "None recorded."),
    "",
    AGENTS_MARKER_END,
    "",
  ].join("\n");
...
function writeAgentsGuide(initPath, metadata, brief, memory) {
  const path = join(initPath, WORKFLOW_AGENTS_FILE);
  const generated = renderAgentsGuide(metadata, brief, memory);
  const existing = readOptionalText(path);

  if (existing && existing.includes(AGENTS_MARKER_START) && existing.includes(AGENTS_MARKER_END)) {
    const start = existing.indexOf(AGENTS_MARKER_START);
    const end = existing.indexOf(AGENTS_MARKER_END) + AGENTS_MARKER_END.length;
    writeFileAtomically(path, `${existing.slice(0, start)}${generated.trim()}${existing.slice(end)}`);
    return path;
  }
  if (existing.trim()) {
    writeFileAtomically(path, `${generated}\n${existing.trim()}\n`);
    return path;
  }
...
function syncAgentsGuide(initPath) {
  const record = readWorkflowRecord(initPath);
  return writeAgentsGuide(initPath, record.metadata, record.brief.context, record.memory.context);
}
...
function updateWorkflowMemory(initPath, updates = {}) {
  ...
  writeAgentsGuide(initPath, metadata, record.brief.context, memory);
...
function updateWorkflowBrief(initPath, updates = {}) {
  ...
  validateWorkflowMetadata(metadata);
  writeFileAtomically(record.paths.metadata, JSON.stringify(metadata, null, 2));
  writeAgentsGuide(initPath, metadata, brief, record.memory.context);
...
function planLegacyMigration(initPath) {
  ...
  return {
    ...
    creates: [
      ...
      ...(!existsSync(join(initPath, WORKFLOW_AGENTS_FILE)) ? [WORKFLOW_AGENTS_FILE] : []),
    ],
    preserves: sources,
    preview: {
      legacyMetadata,
